"""
Main window for WizardCast AR application.
"""
import os
...
class BackgroundImageScreen(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        image_path = os.path.join(os.getcwd(), "assets", "images", "hogwarts.jpg")
...
class IntroSequenceScreen(BackgroundImageScreen):
    ...
    def __init__(self, parent=None):
        ...
        self.parent_window = parent
        self.messages = [
            "Chào mừng đến với WizardCast AR — Nơi bạn học phép thuật bằng cử chỉ tay thực tế.\nWelcome to WizardCast AR — Learn magic spells through real hand gestures.\nWizardCast ARへようこそ — 実際の手のジェスチャーで魔法を学びましょう。",
            "Bạn sẽ được hướng dẫn từng phép thuật. Mỗi chương giới thiệu một phép, sau đó cho bạn thực hành.\nYou'll be guided spell-by-spell. Each chapter introduces a spell, then lets you practice it.\n呪文ごとにガイドされます。各章では呪文を紹介し、練習できます。",
            "Tập trung, thở đều và làm theo hướng dẫn. Phép tiếp theo chỉ xuất hiện khi bạn đã thành thạo phép hiện tại.\nFocus, breathe, and follow the guide. The next spell only appears when you've mastered the current one.\n集中して、呼吸を整え、ガイドに従ってください。現在の呪文を習得したら次の呪文が表示されます。"
        ]
        self.current_index = 0
        self.active_animations = []

        # Setup intro music player
        self.intro_player = QMediaPlayer()
        self.intro_audio = QAudioOutput()
        self.intro_player.setAudioOutput(self.intro_audio)
        self.intro_audio.setVolume(0.5)

        intro_music_path = os.path.join(os.getcwd(), "assets", "sounds", "hedwigs-theme-theme-from-harry-potter.mp3")
        if os.path.exists(intro_music_path):
            self.intro_player.setSource(QUrl.fromLocalFile(intro_music_path))
        else:
    ...
    def advance_message(self):
        if self.current_index < len(self.messages) - 1:
            ...
            # Stop intro music when finishing intro sequence
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("WizardCast AR")
